...
void wm8994_record_main_mic(struct snd_soc_codec *codec)
{
    ...
    if(wm8994->recognition_active == REC_OFF)
    {
        ...
        wm8994_write(codec, 0x0400, 0x00FF);    // AIF1 ADC1 Left Volume   // 3.75 ==> 6.375
        wm8994_write(codec, 0x0401, 0x00FF);    // AIF1 ADC1 Right Volume  // 3.75 ==> 6.375
    	wm8994_write(codec, 0x0410, 0x1800);    // AIF1 DRC 1(5)
    	wm8994_write(codec, 0x0028, 0x0030);    // Input Mixer 2. Single Ended Input. Negative is connected only.
...
#endif
    	msleep(250);    // To remove the pop noise front of the recorded pcm data.
    ...
    else
    {
       ...
       audio_ctrl_mic_bias_gpio(1);
        ...
        //wm8994_write(codec, 0x0018, 0x0113);    // Left Line Input 1&2 volume
        //wm8994_write(codec, 0x0029, 0x0020);    // Input Mixer 3
        //wm8994_write(codec, 0x0400, 0x00FF);    // AIF1 ADC1 Left Volume  
        //wm8994_write(codec, 0x0401, 0x00FF);    // AIF1 ADC1 Right Volume
    	//wm8994_write(codec, 0x0410, 0x1800);    // AIF1 DRC 1(5)
    	//wm8994_write(codec, 0x0028, 0x0030);    // Input Mixer 2. Single Ended Input. Negative is connected only.
    	wm8994_write(codec, 0x0004, 0x0202);    // Power Management 4
    	wm8994_write(codec, 0x0606, 0x0002);    // AIF1 ADC1 Left Mixer Routing. Enable.
    	...
    	wm8994_write(codec, 0x0001, 0x1003);
        wm8994_write(codec, 0x0018, 0x011B);
        wm8994_write(codec, 0x0028, 0x0030);
        wm8994_write(codec, 0x0029, 0x0020);
        wm8994_write(codec, 0x0400, 0x00FF);
        wm8994_write(codec, 0x0401, 0x00FF);
        wm8994_write(codec, 0x0410, 0x1800);
...
//            wm8994_write(codec, 0x0410, 0x3800);    // AIF1 ADC1 Filter
        ...
        wm8994_write( codec, 0x443, 0x05E0);
        wm8994_write( codec, 0x444, 0x0004);
        wm8994_write( codec, 0x440, 0x0112);
#else   // max vol
        /* DRC */
        wm8994_write( codec, 0x441, 0x0847);
        wm8994_write( codec, 0x442, 0x0400);
        wm8994_write( codec, 0x443, 0x0780);
        wm8994_write( codec, 0x444, 0x0004);
...
void wm8994_set_playback_headset(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x0610, 0x01B8);    // DAC1 Left Volume
	wm8994_write(codec, 0x0611, 0x01B8);    // DAC1 Right Volume
	wm8994_write(codec, 0x0420, 0x0000);    // AIF1 DAC1 Filter
}
...
void wm8994_set_voicecall_receiver(struct snd_soc_codec *codec)
{
    ...
    if(loopback_mode == LOOPBACK_MODE_OFF)
    {
        ...
        wm8994_write(codec, 0x0020, 0x0179);    // Left OPGA Volume
        wm8994_write(codec, 0x0021, 0x0179);    // Right OPGA Volume
        wm8994_write(codec, 0x0610, 0x01B6);    // DAC1 Left Volume
        wm8994_write(codec, 0x0611, 0x01B6);    // DAC1 Right Volume
#if 0
        wm8994_write(codec, 0x001F, 0x0000);    // HPOUT2 Volume
#else
        if(wm8994->codec_state & CALL_ACTIVE)
        {
            wm8994_write(codec, 0x001F, 0x0010);//0x0000);    // HPOUT2 Volume
        }
        else
        {
...
void wm8994_set_voicecall_headset(struct snd_soc_codec *codec)
{
    ...
    if(!wm8994->testmode_config_flag)
    {
        ...
        {
            wm8994_write(codec, 0x001A, 0x0110);    // Right Line Input 1&2 Volume
            wm8994_write(codec, 0x0613, 0x01C0);    // DAC2 Right Volume
