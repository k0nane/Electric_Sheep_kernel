...
{
	// MIDAS[2010.07.04]@Check audio clock gated state because of lockup issue
...
static int s3c_idma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		prtd->state &= ~ST_RUNNING;
		s3c_idma_ctrl(LPAM_DMA_STOP);
#if defined CONFIG_MACH_S5PC110_ARIES	// Prevent to play end of camera shutter sound repeatly in 3s. - DW Shim. 2010.07.21
		memset(substream->runtime->dma_area, 0, LP_BUFSIZE);		// Flush kernel buffer.
#endif
		break;
