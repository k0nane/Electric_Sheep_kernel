...
int keyLedOnLevel = 1;  // active keyled
...
#if defined (GP2A_RECOMMENDED_MODE_B1)
...
// active keyled  {{
int lightsensor_KeyLedLevel()
{
    extern int keyLedOn;
    extern int keyLedSigOn;
    extern void keypad_led_onoff_Ext(int OnOff);
	int value = s3c_adc_get_adc_data(ADC_CHANNEL);
    ...
    if(keyLedSigOn && keyLedOnLevel && (keyLedOn==0))
    {
       keypad_led_onoff_Ext(1);
    }

	gprintk(KERN_ERR "keyLedOnLevel = %d \n",keyLedOnLevel); //Temp
	return keyLedOnLevel;
}
// active keyled  }}
...
#endif
	if((backlight_level > 4) && (!lightsensor_test))
	{
...
#endif	
	gprintk("cur_state = %d\n",cur_state); //Temp
	lightsensor_KeyLedLevel(); // active keyled
}
...
static void gp2a_work_func_prox(struct work_struct *work)
{
   ...
   /* Forbid host's interrupt input - Procedure 4 */
// modify_the_proximity_sensor_malfunction   
...
//	hrtimer_cancel(&gp2a->timer);
   ...
   /* Write HYS Register - Procedure 6 */
   ...
   opt_i2c_write((u8)(REGS_HYS),&value);
  	...
  	/* Forcing vout terminal to go high - Procedure 7 */
	value = 0x18;
	opt_i2c_write((u8)(REGS_CON),&value);

	/* enable INT - Procedure 8 */
	enable_irq(gp2a->irq);
...
//	hrtimer_start(&gp2a->timer,ktime_set(light_init_period/2,0),HRTIMER_MODE_REL);
   ...
   if (INT_CLEAR)
   {
      int_val = REGS_CON | (1 << 7);
   }

	/* enabling VOUT terminal in nomal operation - Procedure 9 */
   value = 0x00;
   opt_i2c_write((u8)int_val, &value);
}
#endif
...
void gp2a_chip_init(void)
{
	...
	if (err) 
   {
		printk(KERN_ERR "failed to request GPJ1(4) for LEDA_3.0V control\n");
		return;
	}
...
#if defined (GP2A_RECOMMENDED_MODE_B1)
// modify_the_proximity_sensor_malfunction
     set_irq_type(IRQ_GP2A_INT, IRQ_TYPE_LEVEL_LOW);
#else
...
void gp2a_on(struct gp2a_data *gp2a, int type)
{
   u8 value;
	...
	ktime_t light_polling_time;
	printk(KERN_ERR "[OPTICAL] gp2a_on(%d)\n",type);
   
	if(type == PROXIMITY)
	{
      gprintk("[PROXIMITY] go nomal mode : power on \n");
      
#if 0 //defined (GP2A_RECOMMENDED_MODE_B1)
		value = 0x18;
		opt_i2c_write((u8)(REGS_CON),&value);
...
static int gp2a_opt_suspend( struct platform_device* pdev, pm_message_t state )
{
	...
	if(!proximity_enable)
	{
		...
		s3c_gpio_cfgpin(GPIO_ALS_SDA_28V, S3C_GPIO_INPUT);
		s3c_gpio_cfgpin(GPIO_ALS_SCL_28V, S3C_GPIO_INPUT);
...
static ssize_t proximity_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	char value;
#if defined (GP2A_RECOMMENDED_MODE_B1)
   value = gp2a_get_proximity_value();
   value = (value == 0) ? 1 : 0;
#else
	value = gpio_get_value(GPIO_PS_VOUT);
#endif
