...
#include "logo_rgba32_hvga_portrait.h"  // forte_temp
...
// draw_black_screen {{
void s3cfb_draw_BlackScreen()
{
    #define DUMMY_SIZE 320

    struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
    struct fb_fix_screeninfo *fix;
    struct fb_var_screeninfo *var;
    int i,j;
    char dumyData[DUMMY_SIZE]=
    ...
    for (i = 0; i < pdata->nr_wins; i++) {
        if (i == pdata->default_win) {
            ...
            s3cfb_check_var(var, fbdev->fb[i]);
            s3cfb_set_par(fbdev->fb[i]);
            //s3cfb_draw_logo(fbdev->fb[i]);


            printk(KERN_ERR "fix->line_length:%d, var->yres:%d", fix->line_length, var->yres);
            //memcpy(fbdev->fb[pdata->default_win]->screen_base, \
            //        LOGO_RGB24, fix->line_length * var->yres);
            for (j = 0; j < 480; j++)
            {
                memcpy(fbdev->fb[pdata->default_win]->screen_base + (1280*j), \
                        dumyData, DUMMY_SIZE);
                memcpy(fbdev->fb[pdata->default_win]->screen_base + (1280*j) + DUMMY_SIZE, \
                        dumyData, DUMMY_SIZE);
                memcpy(fbdev->fb[pdata->default_win]->screen_base + (1280*j) + (DUMMY_SIZE*2), \
                        dumyData, DUMMY_SIZE);
                memcpy(fbdev->fb[pdata->default_win]->screen_base + (1280*j) + (DUMMY_SIZE*3), \
                        dumyData, DUMMY_SIZE);
            }
        }
    }
