...
irqreturn_t pmic_irq(int irq, void *dev_id)
{
	...
	disable_irq_nosync(PMIC_IRQ); //Froyo Merge
...
void Set_MAX8998_PM_IRQ(max8998_pm_section_type reg_num, void (*max8998_isr)(void))
{
    ...
    {
        // Read I2C error
        spin_unlock(&pmic_access_lock);//PREVENT fixed
        return;
    }
    if(max8998_isr == 0)
...
void maxim_vac_connect(void)
{
	...
	if(Get_MAX8998_PM_REG(VDCINOK_status))
	{
		...
		{
			if (chg_first_call) // hanapark_DI02
			{
				chg_first_call = 0;
				msleep(500);
			}
...
void maxim_charging_control(unsigned int dev_type  , unsigned int cmd)
{
	...
	{	
		...
		// hanapark_Atlas (TOPOFF = 10%; 2010.05.10)
		reg_buff[0] = (0x0 <<5) |(0x3 << 3) |(0x2<<0) ; // CHG_TOPOFF_TH=10%, CHG_RST_HYS=Disable, AC_FCGH=380mA(0X01) -> 475mA(0X02)
		if(value){
			if (askonstatus)
