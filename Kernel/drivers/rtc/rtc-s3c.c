...
/* yung@disys */
static int s3c_rtc_settime_only(struct device *dev, struct rtc_time *tm)
{
...
#endif
	pr_debug("s3c_rtc_gettime time [%d.%d.%d]:[%d.%d.%d]\n",
		 rtc_tm->tm_year, rtc_tm->tm_mon, rtc_tm->tm_mday,
		 rtc_tm->tm_hour, rtc_tm->tm_min, rtc_tm->tm_sec);
	memcpy(&save_time, rtc_tm, sizeof(struct rtc_time));

	s3c_rtc_settime_only(dev, &save_time); /* <-- s3c_rtc_settime yung@disys */
	return 0;
}
...
//FIXME	max8998_rtc_read_time(&tm);
...
//FIXME	s3c_rtc_settime(&pdev->dev, &tm);  //update from pmic
	/* check rtc time */
