...
#if 0 // fixed shortly LED ON issue when slowly slide open
      // controlled by platform via  "static DEVICE_ATTR(brightness, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, NULL, key_led_control) "
    if(slide_is_closed)
    {
        keypad_led_onoff(0);
    }
...
#if 0   /* Already Low */
    /* all scan lines = LOW */
    cval = KEYCOL_DMASK & ~0x3F3F  ; /* SCAN[0~5] */
    writel(cval, key_base+S3C_KEYIFCOL);
    s3c_gpio_setpin(GPIO_KEYSCAN6, GPIO_LEVEL_LOW) ;  /* SCAN[6] */
    s3c_gpio_setpin(GPIO_KEYSCAN7, GPIO_LEVEL_LOW) ;  /* SCAN[7] */
...
#if 1 //HW Request key LED control by lightsensor and light level
// active keyled  {{
        if(data == 2)
        {
            keyLedSigOn = 0;
...
static int __init s3c_keypad_probe(struct platform_device *pdev)
{
	...
	writel(KEYIFCOL_CLEAR, key_base+S3C_KEYIFCOL);
#if 1 // TODO:FORTE REV01_GPIO_CONTROL
   /* GPIO_CONTROL */
    s3c_gpio_setpin(GPIO_KEYSCAN6, GPIO_LEVEL_LOW) ;
    s3c_gpio_setpin(GPIO_KEYSCAN7, GPIO_LEVEL_LOW) ;
#endif
