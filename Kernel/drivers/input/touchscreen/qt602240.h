...
// For prevent  the message id is 0xff from E-plus
#ifdef CONFIG_TOUCHSCREEN_QT602240_FORTE
    /* S5C_GPIOINT(D0,3) GPD0_3 */
    ...
    #define INT_PEND_BASE       0xE0200A44
    #define INT_BIT_MASK        (1<<5)
#endif
// ...END

//#define IRQ_TOUCH_INT (IRQ_EINT_GROUP18_BASE+5) /* J0_5 */
#define IRQ_TOUCH_INT       (IRQ_EINT_GROUP6_BASE+3)  /* S5C_GPIOINT(D0,3) GPD0_3 */

#undef QT602240_DEBUG

#ifdef QT602240_DEBUG

#define DEBUG               printk(KERN_ERR "[QT] %s/%d\n",__func__,__LINE__)
#define DEBUG_MSG(p, x...)	do { printk("[QT602240]:[%s] ", __func__); printk(p, ## x); } while(0)
#define ENTER_FUNC	        do { printk("[QT602240] +%s\n", __func__); } while(0)
#define LEAVE_FUNC	        do { printk("[QT602240] -%s\n", __func__); } while(0)
