...
#if 1 // TODO:FORTE_B2  def CONFIG_FORTE_VER_B0 // defined CONFIG_ARIES_VER_B0
    if(check_realboard())
        power_config.idleacqint = 48; //2010.10.19 for hw request  r910
    else
...
{
    acquisition_config.chrgtime = 10;//hw bk21.kong request 8-> 10; // 2us -> 2.5us  cause by improvement sensitivity
...
#if 1 // TODO:FORTE_B2  #ifdef CONFIG_FORTE_VER_B0
    touchscreen_config.xsize = 16;//19;
...
if(tsp_version >= 0x15)
	touchscreen_config.blen = 16;//0x20;
else
...
if(tsp_version >= 0x15)
    ...
    touchscreen_config.xrange = 479;   //799;
    touchscreen_config.yrange = 319;   //479;
...
void qt_Grip_Face_Suppression_Config_Init(void)
{
    ...
    gripfacesuppression_config.szthr1 = 30; //2010.10.19 for r910
...
void qt_Noise_Suppression_Config_Init(void)
{
    ...
    noise_suppression_config.freq2 = 0;
     ...
     else {                               // 2010.10.19 for hw request r910
    noise_suppression_config.freq[0] = 15;//29; 	//5;	// 6;//10;
    noise_suppression_config.freq[1] = 20;//34;	//15;	// 11;//15;
    noise_suppression_config.freq[2] = 25;//39;	//25;	//16;//20;
    noise_suppression_config.freq[3] = 40;//49;	//35;	// 19;//25;
    noise_suppression_config.freq[4] = 45;//58;	//45;	// 21;//30;
    }
...
void quantum_touch_probe(void)
{
	...
	else
	{
		...
		QT_printf(KERN_ERR "Version: 	   0x%x\n", tsp_version);
		QT_printf(KERN_ERR "Family:		   0x%x\n", family_id);
		QT_printf(KERN_ERR "Variant: 	   0x%x\n", variant);
		QT_printf(KERN_ERR "Build number:   0x%x\n", build);

		QT_printf(KERN_ERR "Matrix X size : %d\n", info_block->info_id.matrix_x_size);
		QT_printf(KERN_ERR "Matrix Y size : %d\n", info_block->info_id.matrix_y_size);
...
//0413			if((tch_ch>=0) && (atch_ch <= 1))  //jwlee change.
			if((tch_ch>0) && (atch_ch == 0))  //jwlee change.
			{
				...
				if(!check_abs_time())
					qt_time_diff=501;
//				else
//					dprintk( KERN_ERR " CURRENT time diff = %d, qt_timer_state = %d\n", qt_time_diff, qt_timer_state);
...
int set_tsp_for_ta_detect(int state)
{
	...
	if(state)
        {
		touchscreen_config.tchthr = 55;//55(출하검사 윤서희 확인.)->60-> 70; stylus pen touch issue cause by use low price TA from Outgoing Quality Part
...
//					cal_check_flag = 2u;
                ...
                #if 1 // For prevent  the message id is 0xff from E-plus
...
//		  printk(";\n[TSP]Reset Chip!!!\n");
	...
	//			amplitude++;
				//printk( KERN_ERR "[TSP]%s x=%3d, y=%3d\n",__FUNCTION__, fingerInfo[i].x, fingerInfo[i].y);
				//printk( KERN_ERR "ID[%d]= %s", (fingerInfo[i].size_id>>8), (fingerInfo[i].pressure == 0)?"Up ":"" );
	...
	//		printk( KERN_ERR "\n");
	//		printk( KERN_ERR "##### Multi-Touch Event[%d] Done!\n", amplitude );
		}
		nPrevID= id;
#else
...
// For prevent  the message id is 0xff from E-plus
#if 1
    if(readl(gpio_pend_mask_mem)&INT_BIT_MASK)
         writel(readl(gpio_pend_mask_mem)|INT_BIT_MASK, gpio_pend_mask_mem);
#else
...
int qt602240_i2c_read(u16 reg,unsigned char *rbuf, int buf_size)
{
	...
	//	printk( KERN_ERR "%s, %d, %d = ",__func__,data[0], data[1]);
...
//	DEBUG;
	...
	//	INIT_WORK(&qt602240->ts_event_work, qt602240_work_func);
	...
	input_set_abs_params(qt602240->input_dev, ABS_X, 0, 319/*479*/, 0, 0);
	input_set_abs_params(qt602240->input_dev, ABS_Y, 0, 479/*799*/, 0, 0);
#ifdef _SUPPORT_MULTITOUCH_
	input_set_abs_params(qt602240->input_dev, ABS_MT_POSITION_X, 0, 319/*479*/, 0, 0);
	input_set_abs_params(qt602240->input_dev, ABS_MT_POSITION_Y, 0, 479/*799*/, 0, 0);
#endif
...
{
	QT_reprogram();
	msleep(100);   // 2010.11.27 TSP fail after power cycle
}
#else
	quantum_touch_probe();
#endif
...
// TODO:FORTE	printk(KERN_DEBUG "TOUCH_RST : %s\n", gpio_get_value(GPIO_TOUCH_RST)? "High":"Low");
...
void QT_reprogram(void)
{
	...
	printk(KERN_ERR "QT_reprogram check\n");

	if(boot_read_mem(0,1,&rxdata) == READ_MEM_OK)
	{
		printk(KERN_ERR "Enter to new firmware : boot mode\n");
	        if(QT_Boot_no_reset())
	            TSP_Restart();
	        printk(KERN_ERR "Reprogram done : boot mode\n");
	}
	...
	{
	        printk(KERN_ERR "Enter to new firmware : ADDR = Other Version\n");
	        if(QT_Boot())
	        {
	            TSP_Restart();
	            quantum_touch_probe();
	        }
	        printk(KERN_ERR "Reprogram done : ADDR = Other Version\n");
	}
...
/*
	...
	{
		printk(KERN_ERR "Failed to calibrate, exiting...\n");
		return WRITE_MEM_FAILED;
	}
*/
...
//	printk( KERN_ERR "[TSP] disable IRQ.......\n");
	disable_irq(qt602240->client->irq);
	switch(mode)
		{
		case 1:
...
int __init qt602240_init(void)
{
	...
	if(ret) printk(KERN_ERR"[%s], i2c_add_driver failed...(%d)\n", __func__, ret);

	printk(KERN_ERR"[QT] ret : %d, qt602240->client name : %s\n",ret,qt602240->client->name);

	if(!(qt602240->client)){
		printk( KERN_ERR "[%s],slave address changed try to firmware reprogram \n",__func__);
	...
	if(!(qt602240->client)){
		printk( KERN_ERR "###################################################\n");
		printk( KERN_ERR "##                                               ##\n");
		printk( KERN_ERR "##    WARNING! TOUCHSCREEN DRIVER CAN'T WORK.    ##\n");
		printk( KERN_ERR "##    PLEASE CHECK YOUR TOUCHSCREEN CONNECTOR!   ##\n");
		printk( KERN_ERR "##                                               ##\n");
		printk( KERN_ERR "###################################################\n");
		i2c_del_driver(&qt602240_i2c_driver);
		return 0;
...
//	ts_dev = device_create_drvdata(sec_class, NULL, 0, NULL, "ts");
	...
	printk( KERN_ERR "[QT] %s/%d, platform_driver_register!!\n",__func__,__LINE__);
...
//	switch_test = device_create_drvdata(touch_class, NULL, 0, NULL, "switch");
	...
	if (IS_ERR(qt602240_atcom_test))
		printk( KERN_ERR "Failed to create device(qt602240_atcom_test)!\n");
