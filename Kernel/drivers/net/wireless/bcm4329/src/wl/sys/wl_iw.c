...
#ifndef CUSTOMER_HW2
	uint32 temp1=0, temp2=0, temp3=0;//PREVENT fixed
#endif 
...
int hotspot_event_detect_complete(char *msg)
{
	...
	{
		printk("hotspot_polling_flag : %d\n", hotspot_polling_flag);
		return 0;
	}
		

	printk("hotspot_event_detect_complete1 : hotspot_polling_flag : %d\n", hotspot_polling_flag);
	local_save_flags(flags);
	local_irq_disable();
	memset(hotspot_str_buf, 0, sizeof(hotspot_str_buf));
	
	strcpy(hotspot_str_buf, msg);
	printk("hotspot_event_detect_complete2 : %s\n", hotspot_str_buf);
	hotspot_event_flag = 1;

	if(hotspot_readQ_cnt < HOTSPOT_MAX_QUEUE_CNT)
		hotspot_readQ_cnt++;
	printk("hotspot_event_detect_complete3\n");
	local_irq_restore(flags);
	printk("hotspot_event_detect_complete4\n");
...
)
{
	...
	for (i = 0; i < rateset.count && i < WL_NUMRATES /*IW_MAX_BITRATES*/; i++)//PREVENT fixed
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++)
	{
		...
		if (bi->rateset.count) {
			if (((event -extra) + IW_EV_LCP_LEN) <= (uintptr)end) {
				...
				for (j = 0; j < bi->rateset.count && j < WL_NUMRATES/*IW_MAX_BITRATES*/; j++) {//PREVENT fixed
					iwe.u.bitrate.value =
...
int dev_iw_write_cfg1_var(struct net_device *dev, char *cmd, int val)
{
	struct {
		int cfg;
		int val;
	...
	bss_set_res = dev_iw_iovar_setbuf(dev, cmd,
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	WL_SOFTAP(("set_ap_cfg: set ap profile: dev 0x%X '%s'\n", dev, dev->name));
	WL_SOFTAP(("	ssid = '%s'\n", ap->ssid));
	...
	if (ap_cfg_running == FALSE) {
		...
		if ((res = dev_iw_write_cfg1_var(dev, "bss", 0)) < 0) {
...
#endif /* NEW_AP_INTERFACE */
		if ((res = dev_iw_write_cfg1_var(ap_net_dev, "bss", 1)) < 0) {
			...
			int param_type, void  *dst, int param_max_len)
{
	...
	if (!strncmp(*str_ptr, token, strlen(token))) {
		...
		WL_SOFTAP((" 'token:%s', len:%d, ", token, parm_str_len));
		...
		switch (param_type) {
			...
			case PTYPE_INTDEC: {
				...
				*pdst_int = simple_strtoul(int_str, &eptr, 10);
				...
				WL_SOFTAP((" written as integer:%d\n",  *pdst_int));
			}
			break;
			case PTYPE_STR_HEX: {
				u8 *buf = dst;
	...
	char *ext)
{
	...
	if  ((ap_cfg_running == TRUE)) {
		...
		if ((res = dev_iw_write_cfg1_var(dev, "bss", 2)) < 0)
	...
	char *ext)
{
	...
	WL_TRACE((">Got FW_RELOAD cmd:"
				"info->cmd:%x, info->flags:%x, u.data:%p, u.len:%d fw_path:%p, len:%d\n",
				info->cmd, info->flags,
...
#endif /* NEW_AP_INTERFACE */
		if ((err = dev_iw_write_cfg1_var(dev, "bss", 1)) < 0)
			...
			char *cmd_str)
{
	...
	if (strnicmp(sub_cmd, "AP_CFG", strlen("AP_CFG")) == 0) {
		...
		if (ap_net_dev == NULL) {
				 WL_SOFTAP(("ERROR: SOFTAP net_dev* is NULL !!!\n"));
		} else {
		...
		WL_SOFTAP((" \n temp DOWN SOFTAP\n"));
		if ((ret = dev_iw_write_cfg1_var(dev, "bss", 0)) < 0) {
...
)
{
	...
	WL_INFORM(("%s, cmd:%x alled via dhd->do_ioctl()entry point\n", __FUNCTION__, cmd));
	if (cmd < SIOCIWFIRST ||
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_PMKID_CACHE: {
		if (data)
		{
			...
			WL_TRACE(("WLC_E_PMKID_CACHE enter\n"));
			...
			while (count) {
				WL_TRACE(("WLC_E_PMKID_CACHE count : %d\n",count));
