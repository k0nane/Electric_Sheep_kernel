...
#include "cam_s5k5ccgx.h"
#include "cam_s5k5ccgx_regs_evt_1.h"
...
#else
#define CAM_ERROR_MSG(dev, format, arg...)
#define CAM_WARN_MSG(dev, format, arg...)
...
int s5k5ccgx_regs_table_init(void)
{
	...
	filp = filp_open("/sdcard/cam_s5k5ccgx_regs_evt_1.h", O_RDONLY, 0);
...
static int s5k5ccgx_set_preview_size(struct v4l2_subdev *sd)
{
	...
	switch(index)
	{
		...
		case PREVIEW_SIZE_D1:
		{
        	...
        	state->postview_info.width = 480;
        	state->postview_info.height = 320;			                                  
			break;
        }
...
static int s5k5ccgx_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if(!state->pix_format.width || !state->pix_format.height)
	{
...
static int s5k5ccgx_set_jpeg_quality(struct v4l2_subdev *sd)
{
    ...
    CAM_ERROR_MSG(&client->dev, "%s : quality %d set done~~~~~~~~~~~~~~\n", __func__, quality);
...
static int s5k5ccgx_get_low_light_condition(struct v4l2_subdev *sd, int *Result)
{
    ...
    {
        *Result = HIGH_LIGHT;
	    CAM_ERROR_MSG(&client->dev,"%s : Highlight Read(0x%X) \n", __func__, NB_value);
    }
    else if(NB_value > 0x0020)
    {
        *Result = NORMAL_LIGHT;
	    CAM_ERROR_MSG(&client->dev,"%s : Normallight Read(0x%X) \n", __func__, NB_value);
    }
    else
    {
        *Result = LOW_LIGHT;
	    CAM_ERROR_MSG(&client->dev,"%s : Lowlight Read(0x%X) \n", __func__, NB_value);
    }
...
static int s5k5ccgx_set_capture_size(struct v4l2_subdev *sd)
{
    ...
    switch(index)
    {
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0280);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x01E0);  //REG_0TC_CCFG_usHeight
            break;
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0320);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0258);  //REG_0TC_CCFG_usHeight
            break;
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0400);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0300);  //REG_0TC_CCFG_usHeight
            break;		
        }            
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0500);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x03C0);  //REG_0TC_CCFG_usHeight
            break;		
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0640);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x04B0);  //REG_0TC_CCFG_usHeight
            break;		
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0800);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0600);  //REG_0TC_CCFG_usHeight
            break;		
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x01A0);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0140);  //REG_0TC_CCFG_usHeight
            break;		
        }
        ...
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x02D0);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x01E0);  //REG_0TC_CCFG_usHeight
            break;
        }
        ...
        case CAPTURE_SIZE_USER1: /* 2048x1368 */
        {
            err += s5k5ccgx_i2c_write_word(client, 0x002A, 0x0330);
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0800);  //REG_0TC_CCFG_usWidth
            err += s5k5ccgx_i2c_write_word(client, 0x0F12, 0x0558);  //REG_0TC_CCFG_usHeight
            break;		
        }
...
static int s5k5ccgx_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    else
    {
        ...
        else if(light_state == HIGH_LIGHT)
        {
            /* Set Snapshot registers */ 
        	err = s5k5ccgx_i2c_set_config_register(client, 
...
static int s5k5ccgx_set_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    CAM_WARN_MSG(&client->dev, "[%s : %d] WARNING! Not support set ISO\n", __FILE__, __LINE__);
	return 0;
}
...
static int s5k5ccgx_set_touch_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if(ctrl->value == TOUCH_AF_START)
	{
	    ...
	    CAM_ERROR_MSG(&client->dev, "%s: Start AF Pos[%d %d]\n", __func__, FirstWinStartX, FirstWinStartY);
    }
    else
    {
        err  = s5k5ccgx_i2c_write_word(client, 0xFCFC, 0xD000);
    ...
    CAM_ERROR_MSG(&client->dev, "%s: Stop AF Pos\n", __func__);
...
static int s5k5ccgx_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
    ...
    if(ctrl->value == CAMERA_AF_CMD_START) 
    {
        CAM_ERROR_MSG(&client->dev, "%s: AF Start~~~~~~~\n", __func__);		
    ...
    else if(ctrl->value == CAMERA_AF_CMD_STOP) 
    {
        CAM_ERROR_MSG(&client->dev, "%s: AF Stop~~~~~~~\n", __func__);	
        ...
        if(err < 0) 
        {
            CAM_ERROR_MSG(&client->dev, "%s: s5k5ccgx_set_AF_default_position fail \n", __func__);	
            mutex_unlock(&af_cancel_op);
            return -EIO;
        }
    }
...
{   
    ...
    if(!bStartFineSearch)
    {
    	...
    	else
    	{
        	if(AF_status & 0x0002) 
        	{
        		CAM_ERROR_MSG(&client->dev, "%s: AF is success~~~~~~~(Single Search) \n", __func__);
        	...
        	{
        		CAM_ERROR_MSG(&client->dev, "%s: AF is Failure~~~~~~~(Single Search) \n", __func__);
        		ctrl->value = CAMERA_AF_STATUS_FAIL;
        	}
        }
    }
    else // Fine Search
    {
    	...
    	{
    		CAM_ERROR_MSG(&client->dev, "%s: AF is success~~~~~~~(Fine Search) \n", __func__);

    		ctrl->value = CAMERA_AF_STATUS_SUCCESS;
    	}
    	else
    	{
    	    CAM_WARN_MSG(&client->dev, "%s: AF is in progress~~~~~~~(Fine Search) \n", __func__);
...
static int s5k5ccgx_AE_AWB_lock(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->value) 
	{
		case AE_UNLOCK_AWB_UNLOCK:
		{
            ...
            // AWB Unlock
            if(gCurrentWB == WHITE_BALANCE_AUTO 
               && gCurrentScene != SCENE_MODE_SUNSET    // Scene modes using MWB settings.
               && gCurrentScene != SCENE_MODE_DUSK_DAWN
               && gCurrentScene != SCENE_MODE_CANDLE_LIGHT
              )
            {
                err += s5k5ccgx_i2c_write_word(client, 0xFCFC, 0xD000);
                err += s5k5ccgx_i2c_write_word(client, 0x0028, 0x7000);
...
static int s5k5ccgx_get_framesize_index(struct v4l2_subdev *sd)
{
	...
	CAM_ERROR_MSG(&client->dev, "%s: Requested Res: %dx%d\n", __func__, state->pix_format.width, state->pix_format.height);
	...
	for(i = 0; i < (sizeof(s5k5ccgx_framesize_list)/sizeof(struct sensor_enum_framesize)); i++)
	{
		...
		if(state->op_mode == OP_MODE_IMAGE)
		{
			...
			if(frmsize->width == state->pix_format.width && frmsize->height == state->pix_format.height)
			{
		...
		else 
		{
			...
			{
				preview_ratio = state->pix_format.width * 10 / state->pix_format.height;
			}
		}
	} 
    ...
    else
    {
    	...
    	else if(preview_ratio == 12)
    	{
    	    ...
    	    size_index = (state->sensor_mode == SENSOR_MODE_CAMCORDER) ? PREVIEW_SIZE_4CIF : PREVIEW_SIZE_XGA;
    	}
    	else // (preview_ratio == 13) and others.
    	{
    	    // In camcorder mode, VGA size will be used instead of XGA to enhance performance.
    	    size_index = (state->sensor_mode == SENSOR_MODE_CAMCORDER) ? PREVIEW_SIZE_VGA : PREVIEW_SIZE_XGA;
    	}
    }
...
static int s5k5ccgx_set_framesize_index(struct v4l2_subdev *sd, unsigned int index)
{
	...
	for(i = 0; i < (sizeof(s5k5ccgx_framesize_list)/sizeof(struct sensor_enum_framesize)); i++)
	{
		if(s5k5ccgx_framesize_list[i].size_index == index && s5k5ccgx_framesize_list[i].op_mode == state->op_mode)
		{
			...
			state->pix_format.width = s5k5ccgx_framesize_list[i].width;
			state->pix_format.height = s5k5ccgx_framesize_list[i].height;
...
static int s5k5ccgx_s_fmt(struct v4l2_subdev *sd, struct v4l2_format *fmt)
{
	...
	state->pix_format.width = fmt->fmt.pix.width;
	state->pix_format.height = fmt->fmt.pix.height;

	state->pix_format.pixelformat = fmt->fmt.pix.pixelformat;
	...
	CAM_ERROR_MSG(&client->dev, "%s:framesize_index = %d\n", __func__, framesize_index);
	...
	if(state->pix_format.pixelformat == V4L2_PIX_FMT_JPEG)
	{
...
static int s5k5ccgx_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->id) 
	{
		...
		case V4L2_CID_CAMERA_SCENE_MODE:
		{
			...
			CAM_ERROR_MSG(&client->dev,"s5k5ccgx_set_scene_mode = %d \n", ctrl->value);	
			break;
        }
...
static int s5k5ccgx_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	...
	{
		state->pix_format.width = pdata->default_width;
		state->pix_format.height = pdata->default_height;
	}

	if(!pdata->pixelformat)
	{
		state->pix_format.pixelformat = DEFAULT_PIX_FMT;
	}
	else
	{
		state->pix_format.pixelformat = pdata->pixelformat;
	}
