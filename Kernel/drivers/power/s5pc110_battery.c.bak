...
static void check_chg_current(struct power_supply *bat_ps)
{
	...
	time_check = jiffies - start_time_msec;
	if (time_check < (5*HZ))
	...
	if ((s3c_bat_info.bat_info.batt_vol >= FULL_CHARGE_COND_VOLTAGE) &&		// DH11 (full charge condition added)
		(s3c_bat_info.bat_info.batt_current <= CURRENT_OF_FULL_CHG) || detected) {	// hanapark_Atlas (wait 20 sec to notify full charging)
#ifdef __TEST_MODE_INTERFACE__
...
static unsigned int s3c_bat_check_v_f(void)
{
	...
	if (batt_auth_full_check == 0)
	{
		...
		if (!retval)
		{
			printk(KERN_EMERG "%s: Battery auth. full check (Fail) \n", __func__);
		...
		printk(KERN_EMERG "%s: Battery auth. full check (Pass) \n", __func__);
	}
	else
	{
...
//3 DK18 Dr.J TEMP ...		dev_info(dev, "%s: Unauthorized battery!\n", __func__);
		s3c_set_bat_health(POWER_SUPPLY_HEALTH_UNSPEC_FAILURE);
		rc = 0;
	}
