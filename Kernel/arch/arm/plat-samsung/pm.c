...
static int s3c_pm_enter(suspend_state_t state)
{
	...
	/* ensure the debug is initialised (if enabled) */
		...
		//Do not power-off LTE if LTE manufacturing process mode is enabled.
	extern char lte_factory_test;

	if(lte_factory_test == 1)
	{
		printk("====== LteFactoryTest : no sleep =======");
		return 0;
	}
   ...
   /**********************************************************]*/
	...
	/*Set EINT as wake up source*/
       /*  000 = Low Level
           ...
           101 ~ 111 = Resered             */
       /*  0 : AP_PS_HOLD(x)   */
#ifdef CONFIG_OPTICAL_GP2A  //HD.Jeong.
    if(gp2a_get_proximity_enable())
    {
	      // modify_the_proximity_sensor_malfunction
          s3c_pm_set_eint(1, 0x0);    /* VOUT_PS      */
    }
#endif /* OPTICAL_GP2A */
    s3c_pm_set_eint( 2, 0x4);   /* SHORT_SEND_END   */
    s3c_pm_set_eint( 3, 0x3);   /* WLAN_HOST_WAKE   */
    s3c_pm_set_eint( 4, 0x4);   /* BT_WAKE_UP       */
    s3c_pm_set_eint( 5, 0x4);   /* HALL_SW          */
    s3c_pm_set_eint( 6, 0x4);   /* EAR_DET          */
    s3c_pm_set_eint( 7, 0x2);   /* PMIC             */
    // TODO:LTE s3c_pm_set_eint( 8, 0x4);   /* LTE_ACTIVE    */
    s3c_pm_set_eint( 9, 0x3);   /* DPRAM_INT(Rising)*/
    /* 10 : FUEL_GUAGE_ALT  */
    s3c_pm_set_eint(11, 0x2);   /* ONEDRAM_AP_n     */
    /* 12 : CMC_RST(X)          */
    s3c_pm_set_eint(13, 0x4);   /* OPEN_SEND_END    */
    /* 14 : 220_PMIC_PWRON(x)   */
    /* 15 : QSC_ACTIVE          */
    /* 16 ~ 21 : KEYSCAN(x)     */
    s3c_pm_set_eint(22, 0x2);   /* nPOWER           */
    s3c_pm_set_eint(23, 0x2);   /* JACK_nINT        */
    /* 24 ~ 30 : KEYSENSE(x) : nPower Only              */
    #if 0    /* KEY_SENSE */
        s3c_pm_set_eint(24, 0x2);
        s3c_pm_set_eint(25, 0x2);
        s3c_pm_set_eint(26, 0x2);
        s3c_pm_set_eint(27, 0x2);
        s3c_pm_set_eint(28, 0x2);
        s3c_pm_set_eint(29, 0x2);
        s3c_pm_set_eint(30, 0x2);
    #endif
    s3c_pm_set_eint(31, 0x4);   /* T-FLASH_DETECT  */
...
static int s3c_pm_valid_state(suspend_state_t state)
{
	switch (state) {
		case PM_SUSPEND_ON:
