...
#if 1
#define dbg_loga(s, args...) printk(KERN_CRIT "[OneDRAM] <%s:%d> " s, __func__, __LINE__,  ##args)
#define dbg_loge(s, args...) printk(KERN_ERR "[OneDRAM/Err] <%s:%d> " s, __func__, __LINE__,  ##args)
#define dbg_log(s, args...)  printk("[OneDRAM] <%s:%d> " s, __func__, __LINE__,  ##args)
#else
#define dbg_loga(s, args...)
#define dbg_loge(s, args...)
...
static irqreturn_t onedram_irq_handler(int irq, void *data)
{
	...
	list_for_each(l, &h_list.list) {
        h = list_entry(l, struct onedram_handler, list);
		if (h->handler)
...
static ssize_t onedram_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	while (!data)
	{
        if (wait_event_interruptible(od->waitq, (od->mailbox != 0)))
			return -ERESTARTSYS;
...
static ssize_t onedram_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
	struct onedram *od;
...
static int __devinit onedram_probe(struct platform_device *pdev)
{
	...
	r = request_irq(irq, onedram_irq_handler, IRQF_TRIGGER_LOW, "onedram", od);
	if (r) {
