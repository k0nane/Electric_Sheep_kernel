...
#if 1
#define dbg_loga(s, args...) printk(KERN_CRIT "[ModemCtl] <%s:%d> " s, __func__, __LINE__,  ##args)
#define dbg_loge(s, args...) printk(KERN_ERR "[ModemCtl/Err] <%s:%d> " s, __func__, __LINE__,  ##args)
#define dbg_log(s, args...)  printk("[ModemCtl] <%s:%d> " s, __func__, __LINE__,  ##args)
#else
...
static void lte_off(struct modemctl *);
...
static int __devinit modemctl_probe(struct platform_device *pdev)
{
	...
	struct modemctl_platform_data *pdata = NULL;
	...
	r = request_irq(irq, modemctl_irq_handler, flags, "phone_active", mc);
	if (r)
	{
		dev_err(&pdev->dev, "failed to alloc interrupt(%d): reason(%d)\n", irq, r);
		goto err;
	}
	mc->irq = irq;
