...
/*--------------------------------------------------------------------------- 
 * ENABLE CP UPLOAD FEATURE (kadadi.parsh@samsung.com)
 * - CONFIG_KERNEL_DEBUG_SEC defined kernel kconfig
 --------------------------------------------------------------------------*/
//#define ENABLE_CP_UPLOAD_PHASE_I /*CP send magic code 0xABCD00C9 */
		...
		const unsigned char *buf, int len)
{
	...
	if (dpram_write_pending) {
		printk("dpram_write : write pending for release semaphore\n");
		return -EAGAIN;
	}
...
static int return_onedram_semaphore(const char* func)
{
	...
	if(!atomic_read(&onedram_lock)) 
	{
		...
		requested_semaphore++;
		dprintk("PDA is accessing onedram. %d\n", func, requested_semaphore);
	}
...
static int dpram_nvdata_load(struct _param_nv *param)
{
	...
	printk(KERN_DEBUG "[DPRAM] DSG buffer =  %s \n", aDGSBuf);
...
#define CONFIG_ONEDRAM_IPC_4_1

#if defined(CONFIG_ONEDRAM_IPC_4_1)
typedef struct mem_ipc_4_1 {
	u8    info[64];
	u8    fmt_rx[4096];
	u8    fmt_tx[4096];
} t_mem_ipc_4_1;
#endif	//defined(CONFIG_ONEDRAM_IPC_4_1)
...
static void init_hw_setting(void)
{
	...
	{
		writel(0x80808080, S5P_VA_GPIO + 0x0e88);
		readl(S5P_VA_GPIO + 0x0E88);
		writel(0x80808080, S5P_VA_GPIO + 0x0e8c);
		readl(S5P_VA_GPIO + 0x0E8C);
	}
