...
#define SPI_1_SPEED     16000000        //16 Mhz
...
int DRV_SPI_Init(void)
{
	...
	{
		printk(KERN_ERR "[LTESPI] %s:: ioremap falied!!!\n", __func__);
	}
	...
	s3c_gpio_cfgpin(GPIO_LTE_SPI_MOSI, S3C_GPIO_SFN(GPIO_LTE_SPI_MOSI_AF));
	s3c_gpio_setpull(GPIO_LTE_SPI_MOSI, S3C_GPIO_PULL_UP);

    rd_addr = (spi0_regbase + 0);   // ch_cfg
    rd_val = readl(rd_addr) |
             (0 << 6)       |       // High_Speed_En = 0
             (0 << 5)       |       // SW_RST
             (0 << 4)       |       // MASTER
             (0 << 3)       |       // CPOL = 0
...
#if 0
	printk(KERN_ERR "[LTESPI] SPICFG: ch_cfg = 0x%08x, clk_cfg = 0x%08x, mode_cfg = 0x%08x, slave_sel = 0x%08x\n",
            readl(spi0_regbase),
